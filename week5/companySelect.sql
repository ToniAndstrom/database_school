...
select departmentName, count(employeeId) as employees from department
right join employee on departmentNumber=departmentId --'left join' and 'right join' just flip wich table is shown first
group by departmentName;
...
select departmentName, count(employeeId) as employee, min(salary) as min
from department
left join  employee on departmentNumber=departmentId
group by departmentName;

select departmentName, count(employeeId) as employee, coalesce(min(salary),0) as min --coalesce makes null be 0 with 0 in brackets
from department
left join  employee on departmentNumber=departmentId
group by departmentName;
...
else "on the average" end as salariylevel from employee
inner join department on departmentNumber=departmentId;

...
inner join department on employee.departmentID = department.departmentID
inner join workerPosition on employee.positionId = workerPosition.positionId
where firstname = "Peter" and lastname = "Bond";--gets all peter bonds

select firstname, lastname, departmentName, positionName from employee
inner join department on employee.departmentID = department.departmentID
inner join workerPosition on employee.positionId = workerPosition.positionId
where employeeId=6;--gets only one person

insert into employee values(8, "Peter", "Bond", 4000, 3,2);

--can't do with matching columns
...
update workerPosition set positionName="bigboss" where positionName="leader";

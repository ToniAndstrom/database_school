...
create table workerOfDepartment(
    ...
    foreign key (employeeId) references employee (employeeIdId),
    foreign key (positionId) references workerPosition (positionIdId)
);
...
insert into workerPosition values(2,"leader",2,"vice head of the department" );
...
inner join workerOfDepartment on workerOfDepartment.employeeIdt=employee.employeeId
...
ordered by departmentName asc lastname asc firstname asc;
...
select departmentName,  lastname, firstname, positionDescription from employee
natural join workerOfDepartment 
natural join department Id
natural join workerPosition 
ordered by departmentName asc, positionRank asc, lastname asc, firstname asc
